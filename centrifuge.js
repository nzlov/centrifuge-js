...
*/
    if (!this) {
      throw new TypeError(
        'Array.prototype.indexOf() - can not convert "' + this + '" to object'
      );
    }
...
centrifugeProto._jsonp = function(url, params, headers, data, callback) {
  if (headers.length > 0) {
    this._log(
      "Only AJAX request allows to send custom headers, it is not possible with JSONP."
    );
  }
  self._debug("sending JSONP request to", url);

  var callbackName =
    "centrifuge_jsonp_" + Centrifuge._nextJSONPCallbackID.toString();
    ...
    if (xhr.readyState === 4) {
      if (xhr.status === 200) {
        ...
        try {
          ...
          callback(
            true,
            "JSON returned was invalid, yet status code was 200. Data was: " +
              xhr.responseText
          );
        }
        if (parsed) {
...
centrifugeProto._configure = function(configuration) {
  ...
  if (!this._config.user && this._config.user !== "") {
    if (!this._config.insecure) {
      ...
      this._debug(
        "user not found but this is OK for insecure mode - anonymous access will be used"
      );
      this._config.user = "";
    }
  }
  ...
  if (!this._config.appkey && this._config.appkey !== "") {
    if (!this._config.insecure) {
      ...
      this._debug(
        "appkey not found but this is OK for insecure mode - anonymous access will be used"
      );
      this._config.appkey = "";
    }
  }
...
centrifugeProto._refresh = function() {
  ...
  var cb = function(error, data) {
    if (error === true) {
      ...
      self._debug(
        "error getting connection credentials from refresh endpoint",
        data
      );
      ...
      if (
        self._config.refreshAttempts !== null &&
        self._numRefreshFailed >= self._config.refreshAttempts
      ) {
        self._refreshFailed();
        return;
      }
...
centrifugeProto.subscribe = function(channel, events) {
  ...
  if (currentSub !== null) {
    ...
    if (currentSub._isUnsubscribed()) {
      currentSub._noResubscribe = false; // 自己添加的
...
subProto.publish = function(data) {
  ...
  return new Promise(function(resolve, reject) {
    if (self._isUnsubscribed()) {
      reject(
        self._centrifuge._createErrorObject("subscription unsubscribed", "fix")
      );
      return;
    }
    self._promise.then(
...
},{"es6-promise":2,"wolfy87-eventemitter":4}],2:[function(require,module,exports){
